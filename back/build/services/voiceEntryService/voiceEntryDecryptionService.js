...
const decryptFile = (encryptedFilePath) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const key = Buffer.from(env_1.env.ENCRYPTION_KEY, "hex"); // Clé de chiffrement
    const iv = Buffer.from(env_1.env.ENCRYPTION_IV, "hex"); // Vecteur d'initialisation
    const input = fs_1.default.createReadStream(encryptedFilePath);
    const decryptedFileName = path_1.default.basename(encryptedFilePath, ".enc"); // Nom du fichier sans .enc
    const decryptedFilePath = path_1.default.join("uploads", decryptedFileName); // Chemin relatif du fichier déchiffré
